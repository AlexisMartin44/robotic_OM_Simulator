...
bool Svgfile::getAffichageAxes()
{
    return m_afficherAxes;
}

bool Svgfile::getAffichageNoms()
{
    return m_afficherNoms;
}

Svgfile::Svgfile(bool showAxes, bool showNames, std::string _filename, int _width, int _height) :
    m_filename{_filename}, m_width{_width}, m_height{_height}, m_afficherAxes{showAxes}, m_afficherNoms{showNames}
{
...
void Svgfile::addCircle(double x, double y, double r, double ep, double opacite, std::string color)
{
    m_ostrm << "<circle "
            << attrib("cx", x)
            << attrib("cy", y)
...
void Svgfile::addText(double x, double y, std::string text, std::string color)
{
    ...
    m_ostrm << "<text "
            ...
            << attrib("font-size", "20px")
            << attrib("stroke", "yellow")
            << attrib("font-family", "'Trebuchet MS'")
            << ">" << text << "</text>\n";
}
...
void Svgfile::addText(double x, double y, double val, std::string color)
{
    std::ostringstream oss;
    oss << val;
    addTextGrid(x, y, oss.str(), color);
}

void Svgfile::addTextGrid(double x, double y, std::string text, std::string color)
{
    /// <text x="180" y="60">Un texte</text>
    m_ostrm << "<text "
            << attrib("x", x)
...
void Svgfile::addLineGrid(double x1, double y1, double x2, double y2, std::string color)
{
    m_ostrm << "<line "
            << attrib("x1", x1)
            << attrib("y1", y1)
...
void Svgfile::addGrid(double span, bool numbering, std::string color)
{
    ...
    while (y<=m_height)
    {
        addLineGrid(0, y, m_width, y, color);
        if (numbering)
    ...
    while (x<=m_width)
    {
        addLineGrid(x, 0, x, m_height, color);
        if (numbering)
